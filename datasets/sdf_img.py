...
from nnutils.hand_utils import ManopthWrapper, get_nTh, ManopthWrapper_OakInk, get_nTh_oakink
...
class SdfImg(nn.Module):
    ...
    def __init__(self, cfg, dataset, is_train, data_dir='../data/', base_idx=0):
        ...
        self.anno = {
            ...
            'hTo': [],  # torch Float (4, 4)
            'cTh': [],  # torch.Float (4, 4)
            'contact_label': [],  # torch.Float (778)
        }

        self.base_idx = base_idx
        self.data_dir = data_dir

        self.subsample = cfg.DB.NUM_POINTS # 12000
        self.hand_wrapper = ManopthWrapper().to('cpu')
        self.hand_wrapper_oakink = ManopthWrapper_OakInk().to('cpu')
    ...
    def __getitem__(self, idx):
        ...
        hTo = torch.FloatTensor(self.anno['hTo'][idx]) # torch.Size([4, 4])
        hA = torch.FloatTensor(self.anno['hA'][idx]) # torch.Size([45])
        nTh = get_nTh(self.hand_wrapper, hA[None], self.cfg.DB.RADIUS)[0] # torch.Size([4, 4])
        ...
        oSdf = torch.cat([
            ...
            ], dim=0) # torch.Size([24000, 4])
        sample['oSdf'] = oSdf

        hSdf = self.norm_points_sdf(oSdf, hTo) # torch.Size([24000, 4])
        sample['hSdf'] = hSdf

        nPos_sdf = self.sample_unit_cube(nPos_sdf, self.subsample)
        nNeg_sdf = self.sample_unit_cube(nNeg_sdf, self.subsample)
        nSdf = torch.cat([nPos_sdf, nNeg_sdf], dim=0) # torch.Size([24000, 4])
        sample['nSdf'] = nSdf
        ...
        sample['hA'] = self.rdn_hA(hA) # torch.Size([45])

        sample['nTh'] = geom_utils.matrix_to_se3(nTh) # torch.Size([12])
        sample['hTo'] = geom_utils.matrix_to_se3(hTo) # torch.Size([12])
        sample['indices'] = idx + self.base_idx

        sample['cTh'] = geom_utils.matrix_to_se3(self.anno['cTh'][idx].squeeze(0)) # torch.Size([12])
        sample['bbox'] = self.get_bbox(idx)
        sample['cam_f'], sample['cam_p'] = self.get_f_p(idx, sample['bbox']) # torch.Size([2])
        ...
        sample['contact_label'] = self.get_contact(idx)
        return sample

    def get_contact(self, idx):
        contact_label =  torch.FloatTensor(self.anno['contact_label'][idx])
    ...
    def norm_points_sdf(self, obj, nTh):
        ...
        nXyz = mesh_utils.apply_transform(xyz, nTh[None])  # (N, Q, 3) torch.Size([1, 24000, 3])
        _, _, scale = geom_utils.homo_to_rt(nTh)  # (N, 3)
        sdf = sdf * scale[..., 0:1, None]  # (N, Q, 1) -> (N, 3) torch.Size([1, 24000, 1])
        nObj = torch.cat([nXyz, sdf], dim=-1) # torch.Size([1, 24000, 4])
        return nObj[0]
    ...
    def get_f_p(self, idx, bbox):
        cam_intr = self.dataset.get_cam(idx)  # with pixel?? in canvas # torch.Size([3, 3])
        cam_intr = image_utils.crop_cam_intr(cam_intr, bbox, 1) # torch.Size([3, 3])
        f, p = image_utils.screen_intr_to_ndc_fp(cam_intr, 1, 1) # torch.Size([2])
        f, p = self.jitter_fp(f, p) # torch.Size([2])
        return f, p
...
def vis_db():
    ...
    data_loader = build_dataloader(cfg, 'train', jitter, shuffle=False, bs=8)
    device = 'cuda:0'
    ...
    for i, data in enumerate(data_loader):
        ...
        cObj.textures = mesh_utils.pad_texture(cObj, 'yellow')
        ...
        image_utils.save_images(image['image'], osp.join(save_dir, '%d_cObj_%s_%d' % (i, cfg.DB.NAME, jitter)), bg=data['image'], mask=image['mask'], scale=True)
