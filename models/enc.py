...
class ImageSpEnc(pl.LightningModule):
    ...
    def _foward_res(self, x):
        ...
        x = net.layer1(x) # torch.Size([2, 64, 56, 56])
        latent.append(x)
        x = net.layer2(x) # torch.Size([2, 128, 28, 28])
        latent.append(x)
        x = net.layer3(x) # torch.Size([2, 256, 14, 14])
        latent.append(x)
        x = net.layer4(x) # torch.Size([2, 512, 7, 7])
        latent.append(x)
        x = net.avgpool(x) # torch.Size([2, 512, 1, 1])
    ...
    def forward(self, x, mask=None, flow=None):
        ...
        align_corners = True
        latent_sz = latents[0].shape[-2:] # torch.Size([56, 56])
        ...
        latents = torch.cat(latents, dim=1) # torch.Size([2, 960, 56, 56])
        latents = self.z_head(latents) # torch.Size([2, 256, 56, 56])
        glb_latents = self.global_head(glb_latents) # torch.Size([2, 256])
